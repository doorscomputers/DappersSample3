...
Public Class frmReceiveStokcs
    ...
    Public vRecpieces As Integer = 0
    ...
    Sub SendItemtoGrid()
        ...
        Try
            ...
            If ItemLoc = -1 Then
                drgrid.Rows.Add(mgr7.DataObject.Entity.stckid, mgr7.DataObject.Entity.itemdesc, ceQty.Value, mgr7.DataObject.Entity.retail, Amount3)
            Else
                ...
                Dim NewPrice As Decimal = mgr7.DataObject.Entity.retail * ItemCount
    ...
    Private Sub btnPost_Click(sender As Object, e As EventArgs) Handles btnPost.Click
        ...
        Try
            Dim mReceiveID As Integer
            '+The code to Execute the Sequence operation
            Dim spGetReceiveNo As spGetReceiveNoManager
            Dim transpGetReceiptNo As PDSATransaction = New PDSATransaction()
            spGetReceiveNo = New spGetReceiveNoManager()
            transpGetReceiptNo.Add(spGetReceiveNo.DataObject)
            transpGetReceiptNo.Execute()
            'MessageBox.Show(spGetReceiptNo.DataObject.Entity.ReceiptNo.ToString)
            mReceiveID = 0
            mReceiveID = Convert.ToInt32(spGetReceiveNo.DataObject.Entity.ReceiveNo)
            mgrdrheader.Entity.ReceiveNo = mReceiveID
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            If Not String.IsNullOrEmpty(txtItem.Text) Then
                Try
                    ...
                    Dim col1 As stocksCollection
                    mgr7 = New stocksManager()
                    mgr7.DataObject.WhereFilter = stocksData.WhereFilters.barcode
                    mgr7.Entity.barcode = txtItem.Text.Trim
                    col1 = mgr7.BuildCollection()
                    If mgr7.DataObject.RowsAffected > 0 Then
                        nProdID = mgr7.DataObject.Entity.stckid
                        txtItem.Text = mgr7.DataObject.Entity.itemdesc
                        vbocde = mgr7.DataObject.Entity.barcode
                        ceCost.Value = mgr7.DataObject.Entity.retail
                        txtStckid.Text = CStr(mgr7.DataObject.Entity.stckid)
    ...
    Friend Sub SetQuantity()
        Try
            If drgrid.Rows.Count >= 1 Then
                Dim formQty As New frmRecQty
                frmRecQty.ShowDialog()

                drgrid.SelectedRows(0).Cells(2).Value = CInt(frmRecQty.qtyRec)
                drgrid.SelectedRows(0).Cells(4).Value = CDec(CInt(frmRecQty.qtyRec) * CDec(drgrid.SelectedRows(0).Cells(3).Value))
                txtItem.Text = String.Empty
        ...
        Else
            ...
            Try
                ...
                If mgrloaditem.DataObject.RowsAffected > 0 Then
                    ...
                    dgitems.Columns(5).HeaderText = "Retail"
